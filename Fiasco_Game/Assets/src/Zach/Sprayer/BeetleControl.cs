...
public class BeetleControl : MonoBehaviour
{
	...
	*/
	...
	void AttackBehavior () {
		//Two vectors for changing the direction of the other bullets
		Vector2 positive;
		Vector2 negative;
		negative.x = -1;
		negative.y = 0;
		...
		if (bulletTimer == bulletFrequency) 
		{
			...
			bcInst.Shoot(rb.position, playerDirection+negative, bulletSpeed, bulletPrefab);
			bcInst.Shoot(rb.position, playerDirection+positive, bulletSpeed, bulletPrefab);
			bulletTimer = 0;
		} else 
		{
			bulletTimer++;
